...
bool has_changed(bool prev_state, bool curr_state) {
  ...
  #ifdef VERBOSE
  if(changed) {
    printf("Button state has changed\n");
  }
  #endif
...
void update_position(uint *moves) {
  ...
  if(is_valid_pos(next_row, next_col)) {
    ...
    printf("End of board has been reached. Starting from the top.\n");
    #endif
...
void update_board(const char current_player, const uint moves,
  ...
  // being entered
  printf("Player %c input entered at row %d column %d\n", current_player, row, col);
...
void handle_btn2(char *current_player, uint *moves, char (*board)[COLS],
  ...
  if(!is_valid_pos(row, col)) {
    ...
    printf("Invalid selection at row %d column %d", row, col);
  ...
  if(!is_empty_pos(row, col, board)) {
    ...
    // another location."
    printf("Row %d column %d is not empty.\n", row, col);
  ...
  if(is_win(*current_player, board)) {
    ...
    *is_game_over = true;
    ...
    printf("Please press reset button to start the game.\n");
    printf("Waiting for the reset.\n");
